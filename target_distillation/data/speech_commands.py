...
@dataclass
class SpeechCommandsDatabase:
    json_path: str = db_root+"/speech_commands-v0.0.2-5h/database.json"
    num_workers: int = 0


    def __post_init__(self, *args, **kwargs):
        self.class_map = {c: i for i, c in enumerate(speech_commands_classes)}
...
class SpeechCommandsDataset:
    root_path: str = db_root + "/speech_commands-v0.0.2-5h"
    ...
    def __post_init__(self):
        self.db = SpeechCommandsDatabase(
...
@dataclass
class SpeechCommandsLogitsDataset(SpeechCommandsDataset):
    logit_file: Optional[str] = (
        db_root + "/logits/speech_commands_ensemble_logits/database.json"
    )
